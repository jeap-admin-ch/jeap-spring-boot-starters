...
- Add opt-in `@RetryOnAwsJdbcFailover` transaction retries for AWS JDBC Wrapper `FailoverSuccessSQLException` errors
  with SQL state `08S02`. Every attempt runs in a new transaction; unknown transaction outcomes are not retried.
...
- Restore WireMock classes on the compile classpath for the security test-support modules after the Spring Cloud
  upgrade changed the transitive scope of `org.wiremock:wiremock-standalone`.
...
- `jeap-spring-boot-logging-starter`: the auto-configuration no longer fails to start in non-web (non-servlet)
  Spring Boot applications. The servlet-specific `FilterRegistrationBean` bean is now declared in a nested
  `@ConditionalOnWebApplication(SERVLET)` configuration, so introspecting `JeapLoggingAutoconfig` no longer triggers
  `NoClassDefFoundError: jakarta/servlet/Filter` on a classpath without the servlet API.
...
- Added metrics for the token introspection functionality.
...
- Moved jeap-spring-boot-db-migration-starter to its own repository.
