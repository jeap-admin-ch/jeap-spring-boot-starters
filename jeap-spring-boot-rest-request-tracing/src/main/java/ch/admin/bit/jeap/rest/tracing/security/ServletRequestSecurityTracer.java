    ...
    private String getRequestPathPattern(HttpServletRequest request) {
        ...
        if ((fullPath != null) && (contextPath != null)) {
            if (fullPath.startsWith(contextPath)) {
                ...
                log.warn("Unable to determine the request path of the request: Expected request uri '{}' to start with context '{}'.", fullPath, contextPath);
                return null;
            }
        } else {
            log.warn("Unable to determine the request path of the request from request uri '{}' and request context '{}'.", fullPath, contextPath);
            return null;
        }
    }
