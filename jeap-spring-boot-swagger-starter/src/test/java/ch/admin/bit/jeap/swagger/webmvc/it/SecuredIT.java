...
@SpringBootTest(classes = TestConfiguration.class, properties = {"jeap.swagger.status=SECURED", "jeap.swagger.secured.password={noop}secret"})
...
class SecuredIT {
    ...
    @Test
    void swaggerRedirectSecured() throws Exception {
        mockMvc.perform(get(SWAGGER_REDIRECT_URL))
                .andExpect(status().is(401));
    }

    @Test
    void swaggerUISecured() throws Exception {
        mockMvc.perform(get(SWAGGER_UI_URL))
                .andExpect(status().is(401));
    }

    @Test
    void swaggerConfigSecured() throws Exception {
        mockMvc.perform(get(SWAGGER_CONFIG_URL))
                .andExpect(status().is(401));
    }

    @Test
    void openApiSecured() throws Exception {
    ...
    void swaggerRedirect() throws Exception {
        mockMvc.perform(get(SWAGGER_REDIRECT_URL).with(httpBasic("swagger", "secret")))
    ...
    void swaggerConfigUrl() throws Exception {
        mockMvc.perform(get(SWAGGER_CONFIG_URL).with(httpBasic("swagger", "secret")))
    ...
    void swaggerUIReachable() throws Exception {
        mockMvc.perform(get(SWAGGER_UI_URL).with(httpBasic("swagger", "secret")))
                .andExpect(status().isOk())
