...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.DEFINED_PORT, properties = {"server.port=8010"})
@SuppressWarnings("java:S2699") // asserts are in the super class methods, but sonar does not get it
public class MethodSecurityExpressionHandlerCustomizerSimpleRoleWebmvcIT extends AbstractSimpleRoleAuthorizationIT {

    @Autowired
    @SuppressWarnings("SpringJavaInjectionPointsAutowiringInspection")
    private JeapMethodSecurityExpressionHandlerCustomizer customizer;

    protected MethodSecurityExpressionHandlerCustomizerSimpleRoleWebmvcIT(@Value("${server.port}") int serverPort, @Value("${spring.application.name}") String context) {
