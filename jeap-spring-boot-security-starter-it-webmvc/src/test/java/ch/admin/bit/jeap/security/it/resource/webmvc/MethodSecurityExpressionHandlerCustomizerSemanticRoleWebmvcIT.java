...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.DEFINED_PORT, properties = {"server.port=8009"})
@SuppressWarnings("java:S2699") // asserts are in the super class methods, but sonar does not get it
public class MethodSecurityExpressionHandlerCustomizerSemanticRoleWebmvcIT extends AbstractSemanticRoleAuthorizationIT {

    @Autowired
    @SuppressWarnings("SpringJavaInjectionPointsAutowiringInspection")
    private JeapMethodSecurityExpressionHandlerCustomizer customizer;

    protected MethodSecurityExpressionHandlerCustomizerSemanticRoleWebmvcIT(@Value("${server.port}") int serverPort, @Value("${spring.application.name}") String context) {
