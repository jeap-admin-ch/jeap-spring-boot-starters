...
public class AddBrowserSecurityHeadersFilterWithSpringSecurityTest {
    ...
    private static final String CSP_VALUE =
            "default-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline'; font-src 'self'; img-src 'self'; " +
                    "connect-src 'self' http://issuer; " + // Identity provider issuer is added
                    "frame-src 'self' http://issuer; " + // Identity provider issuer is added
    ...
    void expect_filter_headers_to_override_spring_security_headers() {
        ...
        boolean hasSomeConsumerAPICspHeader = get("/some-consumer-api").then()
                .statusCode(200)
                .extract().headers().hasHeaderWithName(CONTENT_SECURITY_POLICY);
